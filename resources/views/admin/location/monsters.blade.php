                ...
                <div class="col-span-12">
                    ...
                    <div class="col-span-12 mt-5">
                        <div class="intro-y block sm:flex items-center h-10">
                            <div class="flex">
                                <a href="#" onclick="return false;" class="btn-green active">Monster</a>
                                <a href="{{ route('admin.location.items', ['id' => $location->id]) }}" class="btn-green">Items</a>
                                <a href="{{ route('admin.location.nps', ['id' => $location->id]) }}" class="btn-green">Nps</a>
                            </div>
                        </div>
                                ...
                                @foreach ($location->monsters as $monster)
                                    <tr class="intro-x">
                                        ...
                                        <td class="w-40">
                                            <div class="flex">
                                                <div class="mt-0.5">
                                                    <img src="{{ asset($monster->image) }}" alt="">
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-center">{{ $monster->attack }}</td>
                                        <td class="text-center">{{ $monster->dodge }}</td>
                                        <td class="text-center">{{ $monster->armor }}</td>
                                        <td class="text-center">{{ $monster->min_dmg }}</td>
                                        <td class="text-center">{{ $monster->max_dmg }}</td>
                                        <td class="w-5 table-report__action">
                                            <a class="flex items-center text-theme-6" href="{{ route('admin.location.monster.delete', ['id' => $location->id, 'monster_id' => $monster->id]) }}">
    ...
    <div id="addMonster" class="modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('admin.location.monster.add', ['id' => $location->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        <div class="col-span-12 sm:col-span-12">
                            <label for="monster_id" class="form-label">Monster</label>
